...
export function StoreManagementPanel({ dictionary }: StoreManagementPanelProps) {
  ...
  const editLogoPreviewUrl = useMemo(() => {
    if (editLogoFile) {
      return URL.createObjectURL(editLogoFile);
    }

    return currentStore?.logo_url ?? "";
  }, [currentStore?.logo_url, editLogoFile]);

  const createLogoPreviewUrl = useMemo(() => {
    if (!createLogoFile) {
      return "";
    }

    return URL.createObjectURL(createLogoFile);
  }, [createLogoFile]);

  useEffect(() => {
    return () => {
      if (editLogoFile && editLogoPreviewUrl.startsWith("blob:")) {
        URL.revokeObjectURL(editLogoPreviewUrl);
      }

      if (createLogoFile && createLogoPreviewUrl.startsWith("blob:")) {
        URL.revokeObjectURL(createLogoPreviewUrl);
      }
    };
  }, [createLogoFile, createLogoPreviewUrl, editLogoFile, editLogoPreviewUrl]);
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <form
          className="rounded-[2rem] border border-sky-100 bg-white p-8 shadow-[0_24px_60px_rgba(59,130,246,0.1)]"
